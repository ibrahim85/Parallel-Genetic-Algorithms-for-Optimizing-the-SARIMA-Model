...
from allgenomes import AllNetworks
...
class Optimizer():
    ...
    def create_population(self, count):
        ...
        for i in range(0, count):
            ...
            network = Network(self.nn_param_choices, {}, self.ids.get_next_ID(), 0, 0, self.ids.get_Gen())
            #print("-create_population-Network- %d",i)
            network.create_random()
            if i == 0:
                #this is where we will store all genomes
                self.master = AllNetworks( network )
            else:
                # Make sure it is unique....
                # print("--is_duplicate-1-")
                if self.master.is_duplicate( network ):
                    #network = self.mutate(network)
                    network.mutate_one_gene()
                #print("--is_duplicate-2-")
            ...
            if i > 0:
                self.master.add_network(network)
    ...
    def evolve(self, pop):
        ...
        for network in pop:
            self.master.set_accuracy(network)
        ...
        for individual in graded[retain_length:]:
            if self.random_select > random.random():
                ...
                gtc = copy.deepcopy(individual)
                
                if self.master.is_duplicate(gtc):
                    gtc.mutate_one_gene()

                gtc.set_generation( self.ids.get_Gen() )
                parents.append(gtc)
                self.master.add_network(gtc)
